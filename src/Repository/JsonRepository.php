...
final class JsonRepository implements PolicyRepositoryInterface
{
    ...
    private function loadFromDirectory(string $path): void
    {
        ...
        throw_if($files === [], RuntimeException::class, 'No JSON files found in directory: '.$path);

        foreach ($files as $file) {
            $contents = file_get_contents($file);

            throw_if($contents === false, RuntimeException::class, 'Failed to read JSON file: '.$file);

            $data = json_decode($contents, true);

            throw_unless(is_array($data), RuntimeException::class, 'Invalid JSON in file: '.$file);
