...
final class YamlRepository implements PolicyRepositoryInterface
{
    ...
    {
        throw_unless(class_exists(Yaml::class), RuntimeException::class, 'symfony/yaml is required to use YamlRepository');
    ...
    {
        throw_unless($this->has($name), PolicyNotFoundException::class, 'Policy not found: '.$name);
    ...
    public function getMany(array $names): array
    {
        ...
        if ($missing !== []) {
            ...
            throw new PolicyNotFoundException('Policies not found: '.$nameList);
        }
    ...
    private function loadFromFile(string $path): void
    {
        ...
        throw_unless(is_file($path), RuntimeException::class, 'Path is not a file: '.$path);
        ...
        if (isset($data['name'])) {
            ...
            throw_unless(is_string($data['name']), RuntimeException::class, 'Invalid policy structure in file: '.$path);
            ...
            foreach ($data as $policyData) {
                ...
                throw_if(!isset($policyData['name']) || !is_string($policyData['name']), RuntimeException::class, 'Invalid policy structure in file: '.$path);
    ...
    {
        throw_unless(is_dir($path), RuntimeException::class, 'Directory not found: '.$path);

        $files = glob($path.'/*.{yml,yaml}', GLOB_BRACE);

        throw_if($files === false, RuntimeException::class, 'Failed to read directory: '.$path);

        throw_if($files === [], RuntimeException::class, 'No YAML files found in directory: '.$path);

        foreach ($files as $file) {
            $data = Yaml::parseFile($file);

            throw_unless(is_array($data), RuntimeException::class, 'Invalid YAML in file: '.$file);

            throw_if(!isset($data['name']) || !is_string($data['name']), RuntimeException::class, 'Invalid policy structure in file: '.$file);
